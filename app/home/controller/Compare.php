...
class Compare extends BaseMall
{
    ...
    public function index()
    {
        ...
        if ($gid_arr) {
            ...
            $condition[] = array('goods_id','in',$gid_arr);
            $goods_listtmp = $goods_model->getGoodsList($condition);
            ...
            for ($j = 0; $j < self::MAXNUM; $j++) {
                $compare_list[$i][] = isset($compare_listtmp[$j]) ? array(
                    'goods_id' => $compare_listtmp[$j]['goods_id'], 
                    'goods_name' => $compare_listtmp[$j]['goods_name'],
                    'goods_image' => $compare_listtmp[$j]['goods_image'],
                ) : '';
            }
            $i++;
            //商品价格
    ...
    public function showcompare()
    {
        ...
        if ($gid_arr) {
            ...
            $condition[] = array('goods_state','=',1);
            $condition[] = array('goods_id','in',$gid_arr);
            $goods_listtmp = $goods_model->getGoodsList($condition, 'goods_id,goods_commonid,goods_name,gc_id,gc_id_1,gc_id_2,gc_id_3,goods_promotion_price,goods_promotion_type,goods_image');
            if ($goods_listtmp) {
                foreach ($goods_listtmp as $k => $v) {
                    $goods_list[$v['goods_id']] = $v;
                }
    ...
    public function checkcompare()
    {
        ...
        if ($gid_arr) {
            ...
            $condition[] = array('goods_state','=',1);
            $condition[] = array('goods_id','in',$gid_arr);
            $goods_list = $goods_model->getGoodsList($condition, 'goods_id,goods_commonid,goods_name,gc_id,gc_id_1,gc_id_2,gc_id_3,goods_promotion_price,goods_promotion_type,goods_image');
            if ($goods_list) {
                foreach ($goods_list as $k => $v) {
